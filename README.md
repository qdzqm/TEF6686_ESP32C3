...
- **5 种屏幕配色**：双击按钮循环切换（经典黑白 → 琥珀复古 → 绿色荧光 → 深蓝冰蓝 → 白纸反色），选定后自动记忆，开机恢复。
- **操作交互**：旋转编码器调整，按键支持**单击**（切换步进/模式）、**双击**（切换屏幕配色）、**长按**（切换波段）。
...
- 开机自动恢复**上次**保存的状态：`当前波段 / 频率 / 步进 / 模式 / 底栏选中项 / 屏幕配色`。
- 存储使用 I2C **EEPROM**（代码针对 24C64 设计），并实现**均匀磨损（wear leveling）**：
  ...
  - 写入前与上一条记录比对，状态无变化则不写。
- 为避免频繁擦写，不同操作采用不同的保存时机（`radio.ino` 顶部可改）：

  | 操作 | 保存时机 | 对应常量（当前值） |
  |---|---|---|
  | 波段切换、步进/模式切换 | 立即保存 | — |
  | 手动调台 | 频率稳定后延时 | `STATE_SAVE_DELAY_MANUAL_MS`（20 秒） |
  | 搜台落台 | 频率稳定后延时 | `STATE_SAVE_DELAY_SEEK_MS`（20 秒） |
  | 双击切换屏幕配色 | 停留该配色满时长后 | `STATE_SAVE_DELAY_THEME_MS`（10 秒） |
